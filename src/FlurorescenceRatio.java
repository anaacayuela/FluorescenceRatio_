...
public class FlurorescenceRatio extends JFrame implements ActionListener,
		ItemListener {
	...
	private FluorescenceRatioCellCntrMarkerVector markerVector;
	private FluorescenceRatioCellCntrMarkerVector currentMarkerVector;
	...
	private FluorescenceRatioCellCntrImageCanvas ic;
	...
	static FlurorescenceRatio instance;

	public FlurorescenceRatio() {
	...
	void populateTxtFields() {
		...
		while (it.hasNext()) {
			...
			FluorescenceRatioCellCntrMarkerVector markerVector = (FluorescenceRatioCellCntrMarkerVector) it
					.next();
	...
	private JRadioButton makeDynRadioButton(int id) {
		...
		markerVector = new FluorescenceRatioCellCntrMarkerVector(id);
	...
	private void initializeImage() {
		...
		if (img == null) {
			...
			Overlay overlay = v139t ? img.getCanvas().getOverlay()
					: null;
			ic = new FluorescenceRatioCellCntrImageCanvas(counterImg, typeVector, this,
					overlay);
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (command.compareTo(INITIALIZE) == 0) {
			...
			if (counterType.equals("Bg"))
				index = 1;
			else if (counterType.equals("R1"))
				index = 2;
			else if (counterType.equals("R2"))
				index = 3;

			// ic.setDelmode(false); // just in case
			currentMarkerVector = (FluorescenceRatioCellCntrMarkerVector) typeVector
					.get(index - 1);
	...
	public void finalMeasure() {
		try {
			...
			File f1 = new File("FluorescenceRatio.txt");
			f1.delete();
	...
	public void reset() {
		...
		while (mit.hasNext()) {
			FluorescenceRatioCellCntrMarkerVector mv = (FluorescenceRatioCellCntrMarkerVector) mit.next();
			mv.clear();
		}
		if (ic != null)
			ic.repaint();
	...
	public void loadMarkers() {
		...
		currentMarkerVector = (FluorescenceRatioCellCntrMarkerVector) typeVector.get(index);
		ic.setCurrentMarkerVector(currentMarkerVector);
	...
	public FluorescenceRatioCellCntrMarkerVector getCurrentMarkerVector() {
		return currentMarkerVector;
	}

	public void setCurrentMarkerVector(FluorescenceRatioCellCntrMarkerVector currentMarkerVector) {
	...
	public static void setType(String type) {
		...
		instance.currentMarkerVector = (FluorescenceRatioCellCntrMarkerVector) instance.typeVector
				.get(index);
